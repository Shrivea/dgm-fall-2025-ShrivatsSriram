...
This is the Bernoulli distribution with parameter $a = \sigma(z)$, where:
- $P(y=1 \mid x) = a$
- $P(y=0 \mid x) = 1 - a$
...
Intuition:
- If the predicted probability for class $j$ is **too low** ($a_j < y_j$), the term $(y_j - a_j)$ is positive, so the gradient update will **increase** the corresponding weights.
- If the model predicts class $j$ too strongly but it is not the true class, $(y_j - a_j)$ becomes negative, so the update will **decrease** those weights.

So the gradient has the same elegant "error $\times$ input" form as binary logistic regression and ADALINE.
