...
- Interpretability helps not only users, but also **system designers**, by improving measurement, modularity, and value alignment.
- Many core challenges (alignment, reasoning, data limits) remain **open research problems**.
...
**Key Observation:**  
The *same* trained model can behave very differently depending on context.  
Pre-training, fine-tuning, and in-context learning change **how the model is used**, not its weights in the same way.
...
- **Explainability ≠ Accuracy:** A highly accurate model can still be unsafe or untrustworthy if we do not understand its behavior.
- **Post-hoc explanations ≠ true understanding:** Tools like LIME or SHAP may look convincing but can fail to reflect the real decision process.
- **Dropping sensitive features ≠ fairness:** Bias can still enter through correlated variables.
...
### Interpretability Categories at a Glance

| Type | Core Idea | Main Limitation |
|----|----|----|
| Post-hoc | Explain predictions after training | May lack fidelity |
| Transparent | Models are interpretable by design | Limited flexibility |
| Mechanistic | Reverse-engineer internals | Hard at large scale |
...
- **Modes of Evaluation:** Doshi-Velez & Kim (2017) formalized three evaluation modes: _Application-grounded_, _Human-grounded_, and _Functionally-grounded_ <d-cite key="doshi2017towards"></d-cite>.
...
**Interpretation:**  
Scaling laws explain *why* performance improves smoothly with size, but they do **not** explain:
- whether the model is aligned,
- whether it is robust,
- or whether its reasoning is reliable.

This gap motivates the need for interpretability at scale.
...
Predictive models usually treat measurements as fixed, but in fields like biomedicine, measurement is costly and active. Interpretability can highlight _what_ we should be measuring.
...
---
**Takeaway:**  
Observed data often reflects **interventions**, not true risk — and models will learn these patterns unless we explicitly account for them.

- _Example:_ A model predicting mortality in pneumonia patients might learn that extremely high Creatinine levels correlate with _lower_ risk <d-cite key="lengerich2025hidden"></d-cite>.
- _Why?_ Because doctors identify those patients as high-risk and give them dialysis. The model learns the outcome of the _intervention_, not the biological risk.
- [cite_start]_Conclusion:_ If we used this biomarker to guide treatment (stopping dialysis because risk looks low), the metric would cease to predict the outcome[cite: 560].
...
6.  **Emotional Value Functions:** Human emotions serve as values that help guide decisions across contexts. Current AI lacks this as rewards are defined externally and change as the problem changes. Can we instill emotions that provide stable rewards across situations?
7.  **Continual Learning:** Current paradigm retrains static models, while human-like intelligence involves continuous adaptation.
8.  **Ethics & Responsibility:** Decisions made by future AI practitioners will have major, possibly societal-level impacts.
9.  **Economic & Societal Impact:** Superhuman models will reshape industries and power dynamics; open debate and responsible innovation are essential.
